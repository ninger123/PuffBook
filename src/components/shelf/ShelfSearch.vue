    ...
    <div class="shelf-search-wrapper"  :class="{'search-top': ifInputClicked, 'hide-shadow': ifHideShadow}">
      ...
      <transition name="shelf-tab-slide-up">
        <div class="shelf-search-tab-wrapper" v-if="ifInputClicked">
          <div class="shelf-search-tab-item" v-for="(item, index) in tabs" :key="index" @click="onTabClick(item)">
            <span class="shelf-search-tab-text" :class="{'is-selected': item.selected}">{{item.text}}</span>
          </div>
        </div>
      </transition>
    </div>
